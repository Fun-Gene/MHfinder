...
class MicrohaplotypeGenerator:
    ...
    def prepare_microhaplotypes(self, input_prefix, snplist, length_threshold, 
        ...
        ld_file = os.path.basename(input_prefix) + '.ld'
        cmd = f'plink --bfile {input_prefix} --extract {snplist} --r2 --ld-window 10000 --ld-window-kb {length_threshold_kb} --ld-window-r2 0 --out {os.path.basename(input_prefix)}'
        ...
        screen_rem_chr.append(anchor_line[0])
        site_group = []
        ...
        for line in infile1:
            ...
            if line[2] == anchor_line[2]:
                ...
                if line[0] not in screen_rem_chr:
                    screen_rem_chr.append(line[0])
                    ld_rem_pos = []
                    ld_rem_pair = []
                    ld_rem_r2 = []
                if len(site_group) >= min_site_number - 1:
                    ...
                    if self._is_not_subset_any(mh_pri, mhs_pri_rem):
                        ...
                        while ld_line and len(ld_line) > 0 and ld_line[0] != front_site_info[1] and ld_line[0] in screen_rem_chr:
